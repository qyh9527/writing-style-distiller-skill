...
所有模式的最终阶段只输出**一份** `<writing_style>` 配置，不得重复。输出顺序：先附属信息（引用出处、质检记录），最后一个代码块为 `<writing_style>` XML，方便用户一键复制。
...
最终输出完成后，**立即删除**本次蒸馏过程中生成的整个中间产物目录：

```
docs/style-output/{timestamp}-{style-name}/
```

删除前无需征求用户确认——中间产物（research-notes、style-diagnosis、style-draft 等）已在各阶段审查门中展示过，终稿已输出到对话中，落盘文件不再有保留价值。
