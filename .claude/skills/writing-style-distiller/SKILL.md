...
在所有模式的最终阶段完成后（包括引用出处、质检记录等附属内容输出之后），**必须额外执行此步骤**：

将最终定稿的 `<writing_style>` （及可选的 `<tone>`）**单独**用一个 XML 代码块包裹输出，不附带任何引用出处、质检记录、选段理由或说明文字。代码块语言标记为 `xml`。
...
纯净输出完成后，**立即删除**本次蒸馏过程中生成的整个中间产物目录：

```
docs/style-output/{timestamp}-{style-name}/
```

删除前无需征求用户确认——中间产物（research-notes、style-diagnosis、style-draft 等）已在各阶段审查门中展示过，纯净版终稿已输出到对话中，落盘文件不再有保留价值。
